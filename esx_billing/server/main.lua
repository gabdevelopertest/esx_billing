ESX = nil
-- keep this in server side in case of dumps -- @author RumDum
-- keep this empty if you do not want to use the discord-logs feature
local webhook = 'Your Webhook'

TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end)

-- @author RumDum
-- event was kept original as it might break the api completely  [[DO NOT CHANGE!]]
RegisterNetEvent('esx_billing:sendBill')
AddEventHandler('esx_billing:sendBill', function(playerId, sharedAccountName, label, amount)
	local _source = source
	print(_source)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	xPlayer.triggerEvent('esx_billing:openConfirmation', _source, playerId, sharedAccountName, label, amount)
end)
-- event that handles notifications (server side)
RegisterNetEvent('esx_billing:notify')
AddEventHandler('esx_billing:notify', function(src, message)
	TriggerClientEvent('esx:showHelpNotification', src, message)
end)
-- event that sends the confirmed bill (ASYNC)
RegisterNetEvent('esx_billing:sendConfirmedBill')
AddEventHandler('esx_billing:sendConfirmedBill', function(playerId, sharedAccountName, label, amount)
		...
		TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
			if account then
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
					...
					local message = {
						{
							["title"] = 'Billing (Accepted)',
							["description"] = '\n' .. 'User: ' .. GetPlayerName(source) .. '\n' .. 'Target: ' .. GetPlayerName(playerId) .. '\n' ..
											  'Amount: ' .. amount .. '\n' .. 
											  'User\'s Job: ' .. xPlayer.getJob().name
						}
					}
